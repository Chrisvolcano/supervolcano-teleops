...
export default function TrainingLibraryPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto dark:bg-[#0a0a0a]">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Training Library</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Curated video corpus for robot training (anonymized)</p>
        </div>
        <button
          onClick={() => fetchTraining()}
          disabled={loading}
          className="px-4 py-2 text-gray-700 dark:text-gray-300 bg-white dark:bg-[#1f1f1f] border border-gray-300 dark:border-[#2a2a2a] rounded-lg hover:bg-gray-50 dark:hover:bg-[#2a2a2a] flex items-center gap-2"
        >
      ...
      {stats && (
        <div className="grid grid-cols-4 gap-4 mb-6">
          <div className="bg-white dark:bg-[#141414] p-4 rounded-lg border border-gray-200 dark:border-[#1f1f1f]">
          ...
          <div className="bg-white dark:bg-[#141414] p-4 rounded-lg border border-gray-200 dark:border-[#1f1f1f]">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">Room Types</div>
          </div>
          <div className="bg-white dark:bg-[#141414] p-4 rounded-lg border border-gray-200 dark:border-[#1f1f1f]">
      ...
      <div className="flex items-center gap-4 mb-6 flex-wrap">
        ...
        <select 
          ...
          className="bg-white dark:bg-[#1a1a1a] border border-gray-300 dark:border-[#2a2a2a] text-gray-900 dark:text-white rounded-lg px-3 py-2 min-w-[160px]"
        >
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-600 dark:text-gray-400">Min Quality:</span>
          <input 
            type="range" 
            min="0" 
            max="100" 
            ...
            className="w-24"
          />
          <span className="text-sm text-gray-600 dark:text-gray-400 w-10">{qualityThreshold}%</span>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <button
              ...
              className={`px-2 py-1 text-xs rounded-full border transition-colors ${
                objectFilter.includes(label) 
                  ...
                  : 'bg-white dark:bg-[#1f1f1f] border-gray-200 dark:border-[#2a2a2a] text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#2a2a2a]'
              }`}
            >
              {label}
            </button>
          ...
          {objectFilter.length > 0 && (
            <button
              ...
              className="px-2 py-1 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 underline"
            >
              Clear filters
            </button>
          )}
      ...
      {/* Error */}
      {error && (
        <div className="mb-4 p-4 bg-red-50 dark:bg-red-500/20 border border-red-200 dark:border-red-500/30 rounded-lg text-red-700 dark:text-red-400">
          {error}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 dark:bg-[#1a1a1a] rounded-lg">
          <GraduationCap className="w-12 h-12 mx-auto text-gray-300 dark:text-gray-600 mb-4" />
          <p className="text-gray-500 dark:text-gray-400">No training videos found</p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Videos are added to training library after approval</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredVideos.map((video, index) => (
            <div
              key={video.id}
              className="bg-white dark:bg-[#141414] rounded-lg border border-gray-200 dark:border-[#1f1f1f] overflow-hidden hover:shadow-md dark:hover:shadow-none transition-shadow cursor-pointer"
                ...
                {video.object_labels && video.object_labels.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2 mt-2">
                    {video.object_labels.slice(0, 6).map((label, i) => (
                      <span key={i} className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs rounded">
                        {label}
                      </span>
                    ))}
                    {video.object_labels.length > 6 && (
                      <span className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 text-xs rounded">
                        +{video.object_labels.length - 6}
                      </span>
                    )}
                  </div>
      ...
      {selectedVideo && selectedVideoIndex !== null && (
        ...
        >
          <div 
            className="bg-white dark:bg-[#141414] rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-[#1f1f1f]"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="font-medium text-gray-900 dark:text-white">{formatDuration(selectedVideo.duration_seconds)}</div>
                </div>
                <div>
                  <div className="text-gray-500 dark:text-gray-400 mb-1">Room Type</div>
                ...
                {selectedVideo.action_types && selectedVideo.action_types.length > 0 && (
                  <div>
                    <div className="text-gray-500 dark:text-gray-400 mb-1">Actions</div>
                    <div className="font-medium text-gray-900 dark:text-white capitalize">
              ...
              {selectedVideo.object_labels && selectedVideo.object_labels.length > 0 && (
                <div>
                  <div className="text-gray-500 dark:text-gray-400 mb-2">Detected Objects ({selectedVideo.object_labels.length})</div>
              ...
              <div className="border-t border-gray-200 dark:border-[#1f1f1f] pt-4 mt-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-sm text-gray-900 dark:text-white">Training Corpus</span>
                </div>
