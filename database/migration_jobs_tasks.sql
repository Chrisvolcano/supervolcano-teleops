...
ALTER TABLE location_preferences ADD COLUMN IF NOT EXISTS job_id VARCHAR(255) REFERENCES jobs(id);
ALTER TABLE location_preferences ADD COLUMN IF NOT EXISTS task_id UUID REFERENCES tasks(id);

-- Migrate existing data: if moment_id exists, move it to task_id
-- If task_id exists (old meaning), move it to job_id
DO $$
BEGIN
    -- If moment_id column exists, copy to task_id
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'location_preferences' AND column_name = 'moment_id') THEN
        UPDATE location_preferences 
        SET task_id = moment_id::uuid 
        WHERE moment_id IS NOT NULL;
        ALTER TABLE location_preferences DROP COLUMN IF EXISTS moment_id;
    END IF;
    
    -- If old task_id exists (referring to jobs), copy to job_id
    IF EXISTS (SELECT 1 FROM information_schema.columns 
